                        ...
                        <div class="module-access-list">
                            @foreach($modules as $module)
                                @php
                                    ...
                                    $hasAccess = $modulePerms ? ($modulePerms['can_view'] ?? false) : false;
                                @endphp
                                <div class="module-item d-flex align-items-center justify-content-between py-2 border-bottom">
                                    ...
                                    <div class="form-check form-switch">
                                        <input type="checkbox" class="form-check-input" role="switch"
                                            ...
                                            {{ old("modules.{$module->id}.has_access", $hasAccess) ? 'checked' : '' }}>
                                    </div>
                                </div>
                                @if($module->children && $module->children->count() > 0)
                                    @foreach($module->children as $child)
                                        @php
                                            ...
                                            $childHasAccess = $childPerms ? ($childPerms['can_view'] ?? false) : false;
                                        @endphp
                                        <div class="module-item d-flex align-items-center justify-content-between py-2 border-bottom ps-4 bg-light">
                                            ...
                                            <div class="form-check form-switch">
                                                <input type="checkbox" class="form-check-input" role="switch"
                                                    ...
                                                    id="module_{{ $child->id }}"
                                                    {{ old("modules.{$child->id}.has_access", $childHasAccess) ? 'checked' : '' }}>
                                            </div>
                                        </div>
                                    @endforeach
                                @endif
